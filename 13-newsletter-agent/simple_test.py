...
from newsletter_agent.formatter_tools import format_newsletter
...
def main():
    print("\n=== AI in Gaming Newsletter Generator ===\n")
    ...
    # Step 5: Generate newsletter introduction
    print("\nStep 5: Generating newsletter introduction...")
    result = generate_intro("AI in Gaming Weekly", "professional", context)
    print(f"  Introduction generated")
    print(f"  \"{result['intro']}\"")
    
    # Step 6: Format newsletter
    print("\nStep 6: Formatting newsletter as markdown...")
    result = format_newsletter("markdown", context)
    print(f"  {result['message']}")
    
    # Save the newsletter to a file
    newsletter_file = f"newsletter_{datetime.now().strftime('%Y%m%d')}.md"
    with open(newsletter_file, "w") as f:
        f.write(context.state.get("formatted_newsletter", "No newsletter generated"))
    ...
    print("\nNewsletter Preview:")
    print("-" * 80)
    preview = context.state.get("formatted_newsletter", "No newsletter generated")
