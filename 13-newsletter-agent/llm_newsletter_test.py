...
if api_key:
    ...
    print("WARNING: GOOGLE_API_KEY not found in environment variables. LLM formatting will not work.")
...
def main():
    print("\n=== AI in Gaming Newsletter Generator (LLM Format) ===\n")
    
    # Check if Google API key is available
    if not os.getenv("GOOGLE_API_KEY"):
        print("ERROR: GOOGLE_API_KEY is required for LLM formatting.")
        print("Please set the GOOGLE_API_KEY environment variable.")
        print("You can create a .env file with: GOOGLE_API_KEY=your_api_key_here")
        return
    ...
    # Step 6: Generate newsletter with LLM
    print("\nStep 6: Generating newsletter with LLM...")
    result = generate_newsletter_with_llm(context)
    print(f"  {result['message']}")
    newsletter_content = context.state.get("llm_newsletter")
    newsletter_file = f"newsletter_llm_{datetime.now().strftime('%Y%m%d')}.md"
    
    # Save the newsletter to a file
    with open(newsletter_file, "w") as f:
        f.write(newsletter_content or "No newsletter generated")
    ...
    print("\nNewsletter Preview:")
    print("-" * 80)
    preview = newsletter_content or "No newsletter generated"
